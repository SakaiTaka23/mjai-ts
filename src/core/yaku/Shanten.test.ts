...
const cases = arr.map((v) => {
  const expectValue = v[v.length - 1];
  const tiles = v.slice(0, -1);
  const input: HaiArr = [
    Array(9).fill(0) as [Hai, Hai, Hai, Hai, Hai, Hai, Hai, Hai, Hai],
    Array(9).fill(0) as [Hai, Hai, Hai, Hai, Hai, Hai, Hai, Hai, Hai],
    Array(9).fill(0) as [Hai, Hai, Hai, Hai, Hai, Hai, Hai, Hai, Hai],
    Array(7).fill(0) as [Hai, Hai, Hai, Hai, Hai, Hai, Hai],
  ];
  for (const vv of tiles) {
    input[Math.floor(vv / 9)][vv % 9]++;
  }
  return { input, expect: expectValue };
});

describe('syanten', () => {
  it('should calculate correct syanten for all cases', () => {
    let error = 0;

    console.time('syanten test');

    for (const v of cases) {
      try {
        expect(syanten(v.input)).toEqual(v.expect);
      } catch (e) {
        error++;
        console.log('Input:', v.input, 'Expected:', v.expect, 'Got:', e);
      }
    }

    console.timeEnd('syanten test');
