...
{
    "国士無双十三面待ち":{"yakuman":2, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return check13(o.haiArray) && o.hai.reduce((total, v)=>{
            return v === o.agari ? ++total : total
        }, 0) === 2
    }},
    "国士無双":{"yakuman":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return check13(o.haiArray) && o.hai.reduce((total, v)=>{
            return v === o.agari ? ++total : total
        }, 0) === 1
    }},
    "純正九蓮宝燈":{"yakuman":2, "isMenzenOnly":true, "check":(o: Riichi)=>{
    ...
    "九蓮宝燈":{"yakuman":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        let i = MPSZ.indexOf(o.agari[1])
        let arr = o.haiArray[i].concat()
        if (arr[0] < 3 || arr[8] < 3 || arr.includes(0))
            return false
    ...
    "四暗刻単騎待ち":{"yakuman":2, "isMenzenOnly":true, "check":(o: Riichi)=>{
        let res = 0
        for (let v of o.currentPattern) {
    ...
    "四暗刻":{"yakuman":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        let res = 0
        for (let v of o.currentPattern) {
            if (typeof v === 'string' && v === o.agari)
                return false
    ...
    "字一色":{"yakuman":1, "check":(o: Riichi)=>{
        let allow = ['1z', '2z', '3z', '4z', '5z', '6z', '7z']
        return checkAllowed(o, allow)
    }},
    "緑一色":{"yakuman":1, "check":(o: Riichi)=>{
        let allow = ['2s', '3s', '4s', '6s', '8s', '6z']
        return checkAllowed(o, allow)
    }},
    "清老頭":{"yakuman":1, "check":(o: Riichi)=>{
        let allow = ['1m', '9m', '1p', '9p', '1s', '9s']
        return checkAllowed(o, allow)
    }},
    "四槓子":{"yakuman":1, "check":(o: Riichi)=>{
        let res = 0
        for (let v of o.currentPattern)
    ...
    "天和":{"yakuman":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return o.extra.includes('t') && o.isTsumo && o.isOya && !o.furo.length
    }},
    "地和":{"yakuman":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return o.extra.includes('t') && o.isTsumo && !o.isOya && !o.furo.length
    }},
    "人和":{"yakuman":1, "isMenzenOnly":true, "isLocal":true, "check":(o: Riichi)=>{
        return o.extra.includes('t') && !o.isTsumo && !o.isOya && !o.furo.length
    }},
    "大七星":{"yakuman":1, "isMenzenOnly":true, "isLocal":true, "check":(o: Riichi)=>{
        let allow = ['1z', '2z', '3z', '4z', '5z', '6z', '7z']
        return checkAllowed(o, allow) && YAKU['七対子'].check(o)
    }},
    "清一色":{"han":6, "isFuroMinus":true, "check":(o: Riichi)=>{
        let must = o.agari[1]
        let allow = []
        for (let i = 1; i <= 9; i++)
            allow.push(i + must)
    ...
    "混一色":{"han":3, "isFuroMinus":true, "check":(o: Riichi)=>{
        let allow = ['1z', '2z', '3z', '4z', '5z', '6z', '7z']
        let d = ''
        for (let v of o.hai) {
    ...
    "二盃口":{"han":3, "isMenzenOnly":true, "check":(o: Riichi)=>{
        let arr = []
        for (let v of o.currentPattern) {
            if (typeof v === 'string')
                continue
    ...
    "混老頭":{"han":2, "check":(o: Riichi)=>{
        let allow = ['1m', '9m', '1p', '9p', '1s', '9s', '1z', '2z', '3z', '4z', '5z', '6z', '7z']
        return checkAllowed(o, allow)
    }},
    "三槓子":{"han":2, "check":(o: Riichi)=>{
        let res = 0
        for (let v of o.currentPattern)
            if (typeof v !== 'string' && (v.length === 2 || v.length === 4))
                res++
    ...
    "七対子":{"han":2, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return check7(o.haiArray) && !YAKU['二盃口'].check(o)
    }},
    "ダブル立直":{"han":2, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return o.extra.includes('w') && !o.furo.length
    }},
    "一気通貫":{"han":2, "isFuroMinus":true, "check":(o: Riichi)=>{
    ...
    "三色同順":{"han":2, "isFuroMinus":true, "check":(o: Riichi)=>{
    ...
    "平和":{"han":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        let hasAgariFu = true
        for (let v of o.currentPattern) {
            if (typeof v === 'string')  {
    ...
    "一盃口":{"han":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        if (YAKU['二盃口'].check(o))
            return false
        for (let i in o.currentPattern) {
            ...
            if (v.length === 3 && v[0] != v[1]) {
                while (iInt < 4) {
                    iInt++
                    if (
                        Array.isArray(o.currentPattern[i]) &&
                        v.length === o.currentPattern[i].length &&
                        v.every((val, idx) => val === o.currentPattern[i][idx])
                    ) {
                        return true
                    }
                }
    ...
    "門前清自摸和":{"han":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return o.isTsumo
    }},
    "立直":{"han":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return (YAKU['一発'].check(o) || (o.extra.includes('r') || o.extra.includes('l'))) && !YAKU['ダブル立直'].check(o)
    }},
    "一発":{"han":1, "isMenzenOnly":true, "check":(o: Riichi)=>{
        return o.extra.includes('i') || o.extra.includes('y')
    }},
    "嶺上開花":{"han":1, "check":(o: Riichi)=>{
        let hasKantsu = false
        for (let v of o.furo) {
